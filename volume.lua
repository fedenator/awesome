...
local PATH_TO_ICONS = "/home/fpalacios/.config/awesome/themes/default/volume/"
local volume_icon_name="high-volume"
...
local function parse_output(stdout)
    ...
    elseif (level < 50) then
        volume_icon_name="low-volume"
    elseif (level < 75) then
        volume_icon_name="medium-volume"
    else
        volume_icon_name="high-volume"
    end
    return level.."%"
end
...
--{{{ Widget creation
    volume.widget = wibox.widget {
        wibox.widget {
            id = "icon",
            image = PATH_TO_ICONS .. "high-volume.svg",
            resize = false,
